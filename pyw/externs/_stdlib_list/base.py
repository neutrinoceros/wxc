...
def stdlib_list(version=None):
    ...
    # This block is modified from the original version -------------------
    # it should work the same, but this works better as a vendored package
    module_list_file = Path.joinpath(
        Path(__file__).parent, "lists", f"{version}.txt"
    )

    data = module_list_file.read_bytes().decode()
